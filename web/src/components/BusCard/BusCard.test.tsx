...
describe("BusCard", () => {
  it("Should render the bus time data properly", () => {
    ...
    expect(screen.getByTestId("bus-number").innerHTML).toBe(
      String(testBus.busId)
    );
    expect(screen.getByTestId("bus-destination").innerHTML).toBe(
      testBus.destination
    );
    expect(screen.getByTestId("bus-mins-to-arrival").innerHTML).toBe(
      `${testBus.minutesUntilArrival} mins`
    );
  });
